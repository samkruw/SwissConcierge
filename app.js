...
async function generateStayPlan() {
  ...
  prog.textContent = 'Generiere Plan...';

  // Kompakter Prompt - max 2 Tage detail, Rest kuerzer
  const maxDays = Math.min(days, 5); // Max 5 Tage im Detail
  const prompt =
    'Aufenthaltsplan in "'+langName+'" fuer '+loc.name+', '+loc.address+'.' +
    ' '+nights+' Naechte, Check-in '+checkinTime+', Check-out '+checkoutTime+'.' +
    (interests.length ? ' Interessen: '+interests.join(', ')+'.' : '') +
    (notes ? ' Hinweis: '+notes+'.' : '') +
    ' '+wetterInfo +
    '\n\nFormat (EXAKT einhalten):' +
    '\nTag 1: [Datum/Beschreibung]' +
    '\n- Vormittag: [Ort/Aktivitaet]: [1 Satz]' +
    '\n- Mittagessen: [Restaurant]: [1 Satz]' +
    '\n- Nachmittag: [Ort/Aktivitaet]: [1 Satz]' +
    '\n- Abendessen: [Restaurant]: [1 Satz]' +
    '\nTag 2: ...' +
    (maxDays < days ? '\n(weitere '+( days-maxDays)+' Tag(e) analog)' : '') +
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      headers: { 'Authorization': 'Bearer '+key, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model: 'llama-3.1-8b-instant', // Schnelles Modell fuer Plan
        max_tokens: 500,
